...
public class PaleontologistMoglingEntity extends CreatureEntity {
    public PaleontologistMoglingEntity(EntityType<? extends CreatureEntity> type, World worldIn) {
        super(type, worldIn);
    }
    ...
    public ActionResultType getEntityInteractionResult(PlayerEntity player, Hand hand) {
        ...
        if (itemstack.getItem() == Items.LEAD && this.canBeLeashedTo(player)) {
            ...
            } else if(itemstack.getItem() == ItemInit.CORRUPTED_FOSSIL_CHUNK.get()) {
                double rand = Math.random();
                itemstack.shrink(1);
                if(rand <= 0.5) {
                    this.entityDropItem(ItemInit.CORRUPTED_BONE.get());
                }
